...
const ParcelSubnumber3 = () => {
  const { language, setParcelSubnumber3 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Podbroj parcele 3"
        id="floating_filled_parcel_subnumber_3"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelSubnumber3(e.target.value);
        }}
        className={`${
      ...
      />
      <label
        htmlFor="floating_filled_parcel_subnumber_3"
        className={`${
          error.length > 0 && value.length > 0
...
export default ParcelSubnumber3;
