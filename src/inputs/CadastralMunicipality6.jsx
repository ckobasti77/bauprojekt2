...
const CadastralMunicipality3 = () => {
  const { language, setCadastralMunicipality6 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-full z-0 group h-[75px]">
      <input
        type="text"
        name="Katastarska opština 6"
        id="floating_filled_cadastral_muncipality_6"
        value={value}
        onChange={(e) => {
      ...
      />
      <label
        htmlFor="floating_filled_cadastral_muncipality_6"
        className={`${
          error.length > 0 && value.length > 0
