...
const ParcelNumber3 = () => {
  const { language, setParcelNumber6} = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Broj parcele 6"
        id="floating_filled_parcel_number_6"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelNumber6(e.target.value);
        }}
        className={`${
      ...
      />
      <label
        htmlFor="floating_filled_parcel_number_6"
        className={`${
          error.length > 0 && value.length > 0
