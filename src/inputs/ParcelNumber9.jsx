...
const ParcelNumber3 = () => {
  const { language, setParcelNumber9} = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Broj parcele 9"
        id="floating_filled_parcel_number_9"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelNumber9(e.target.value);
        }}
      ...
      />
      <label
        htmlFor="floating_filled_parcel_number_9"
        className={`${
          error.length > 0 && value.length > 0
