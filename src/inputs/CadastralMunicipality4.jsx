...
const CadastralMunicipality3 = () => {
  const { language, setCadastralMunicipality4 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-full z-0 group h-[75px]">
      <input
        type="text"
        name="Katastarska opština 4"
        id="floating_filled_cadastral_muncipality_4"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setCadastralMunicipality4(e.target.value);
        }}
        onBlur={(e) => {
      ...
      />
      <label
        htmlFor="floating_filled_cadastral_muncipality_4"
        className={`${
          error.length > 0 && value.length > 0
