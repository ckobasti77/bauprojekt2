...
const CadastralMunicipality3 = () => {
  const { language, setCadastralMunicipality10 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-full z-0 group h-[75px]">
      <input
        type="text"
        name="Katastarska opština 10"
        id="floating_filled_cadastral_muncipality_10"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setCadastralMunicipality10(e.target.value);
      ...
      />
      <label
        htmlFor="floating_filled_cadastral_muncipality_10"
        className={`${
          error.length > 0 && value.length > 0
