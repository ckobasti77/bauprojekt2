...
const CadastralMunicipality3 = () => {
  const { language, setCadastralMunicipality7 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-full z-0 group h-[75px]">
      <input
        type="text"
        name="Katastarska opština 7"
        id="floating_filled_cadastral_muncipality_7"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setCadastralMunicipality7(e.target.value);
      ...
      />
      <label
        htmlFor="floating_filled_cadastral_muncipality_7"
        className={`${
          error.length > 0 && value.length > 0
