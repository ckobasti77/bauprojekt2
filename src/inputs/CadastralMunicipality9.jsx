...
const CadastralMunicipality3 = () => {
  const { language, setCadastralMunicipality9 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-full z-0 group h-[75px]">
      <input
        type="text"
        name="Katastarska opština 9"
        id="floating_filled_cadastral_muncipality_9"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setCadastralMunicipality9(e.target.value);
        }}
        onBlur={(e) => {
      ...
      />
      <label
        htmlFor="floating_filled_cadastral_muncipality_9"
        className={`${
          error.length > 0 && value.length > 0
