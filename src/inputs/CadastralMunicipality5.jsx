...
const CadastralMunicipality3 = () => {
  const { language, setCadastralMunicipality5 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-full z-0 group h-[75px]">
      <input
        type="text"
        name="Katastarska opština 5"
        id="floating_filled_cadastral_muncipality_5"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setCadastralMunicipality5(e.target.value);
        }}
        onBlur={(e) => {
      ...
      />
      <label
        htmlFor="floating_filled_cadastral_muncipality_5"
        className={`${
          error.length > 0 && value.length > 0
