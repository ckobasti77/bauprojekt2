...
const ParcelNumber3 = () => {
  const { language, setParcelNumber4} = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Broj parcele 4"
        id="floating_filled_parcel_number_4"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelNumber4(e.target.value);
        }}
      ...
      />
      <label
        htmlFor="floating_filled_parcel_number_4"
        className={`${
          error.length > 0 && value.length > 0
