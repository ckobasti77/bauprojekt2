...
const ParcelNumber3 = () => {
  const { language, setParcelNumber5} = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Broj parcele 5"
        id="floating_filled_parcel_number_5"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelNumber5(e.target.value);
        }}
        className={`${
      ...
      />
      <label
        htmlFor="floating_filled_parcel_number_5"
        className={`${
          error.length > 0 && value.length > 0
