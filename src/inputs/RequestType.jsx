  ...
  return (
    <>
      ...
      <div className='flex gap-x-4 w-full justify-between'>
          <div className='w-3/6'>
            <input type="radio" name="Vrsta zahteva" value={language === 'cir' ? 'Стандардан' : 'Standardan'} id="standardan" className='vrsta-zahteva-inputs hidden h-0 w-0'/>
          ...
          <div className="w-3/6">
            <input type="radio" name="Vrsta zahteva" value={language === 'cir' ? 'Хитан' : 'Hitan'}  id="hitan" className='vrsta-zahteva-inputs hidden h-0 w-0'/>
      ...
      {requestType === null && <span className="mx-auto font-bold my-1 text-[#ee3b47] text-center block text-sm">{language === 'cir' ? '* Обавезно' : '* Obavezno'}</span>}
    </>
  )
}
