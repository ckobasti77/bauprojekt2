...
const ParcelNumber3 = () => {
  const { language, setParcelNumber8} = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Broj parcele 8"
        id="floating_filled_parcel_number_8"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelNumber8(e.target.value);
        }}
      ...
      />
      <label
        htmlFor="floating_filled_parcel_number_8"
        className={`${
          error.length > 0 && value.length > 0
