...
const ParcelSubnumber = () => {
  const { language, setParcelSubnumber10 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Podbroj parcele 10"
        id="floating_filled_parcel_subnumber10"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelSubnumber10(e.target.value);
        }}
      ...
      />
      <label
        htmlFor="floating_filled_parcel_subnumber10"
        className={`${
          error.length > 0 && value.length > 0
