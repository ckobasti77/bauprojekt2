...
const ParcelSubnumber = () => {
  const { language, setParcelSubnumber9 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Podbroj parcele 9"
        id="floating_filled_parcel_subnumber9"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelSubnumber9(e.target.value);
        }}
        className={`${
      ...
      />
      <label
        htmlFor="floating_filled_parcel_subnumber9"
        className={`${
          error.length > 0 && value.length > 0
