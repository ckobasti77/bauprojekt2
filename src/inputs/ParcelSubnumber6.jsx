...
const ParcelSubnumber = () => {
  const { language, setParcelSubnumber6 } = useContext(ContextAll);
  ...
  return (
    <div className="relative w-3/6 z-0 group">
      <input
        type="number"
        name="Podbroj parcele 6"
        id="floating_filled_parcel_subnumber6"
        value={value}
        onChange={(e) => {
          setValue(e.target.value);
          setParcelSubnumber6(e.target.value);
        }}
        className={`${
      ...
      />
      <label
        htmlFor="floating_filled_parcel_subnumber6"
        className={`${
          error.length > 0 && value.length > 0
